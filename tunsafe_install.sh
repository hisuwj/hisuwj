...
add_user(){
    ...
    sed -i 's%^Address.*$%'"Address = 192.168.3.$newnum\/24 "'%' $newname.conf

cat >> /etc/tunsafe/TunSafe.conf <<-EOF
[Peer]
PublicKey = $(cat tempubkey)
AllowedIPs = 192.168.3.$newnum/32
EOF
    tunsafe set tun0 peer $(cat tempubkey) allowed-ips 192.168.3.$newnum/32
